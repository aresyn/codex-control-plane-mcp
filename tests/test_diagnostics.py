...
class McpDefinitionTests(unittest.TestCase):
    ...
    def test_health_summary_contains_version_contract_block(self) -> None:
        ...
        self.assertEqual("codex-mcp-agent-guide/v1", version["guideVersion"])
        self.assertEqual(64, len(version["guideHash"]))
        self.assertEqual("startup", version["recommendedStartupFlow"])
        self.assertEqual("codex_health_summary", version["recommendedStartupTool"])
        self.assertEqual("codex_submit_task", version["recommendedPrimaryWriteTool"])
